...
import uvicorn
import botbeer.tool.html_img as html_img
...
import botbeer.tool.dto as dto
...
是否使用云湖hook = bot_config.get("云湖", False)
...
@app.post("/yunhu")
async def yunhu_普通消息(request: Request):
    data: dict = await request.json()
    if not data or not 是否使用云湖hook:
        raise HTTPException(status_code=400, detail="未知请求")
    ...
    if 事件["chat"]["chatType"] == "group":
        msg = 事件["message"]["content"]["text"]
        id = 事件["sender"]["senderId"]
        m = MessageHandler("云湖_群")
        m.id = id
        m.发言 = yunhu.yunhuapi()
        m.发言.data = data
        if 事件["message"]["contentType"] == "markdown":
            await m.特殊_msg(f"MD={msg}")
        else:
            await m.at_msg(msg)
...
async def html_to_img(html: str = None):
    ...
    image_data = await html_img.HTML画图(html)
