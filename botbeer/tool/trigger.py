...
import urllib
from botbeer.tool import html_img
import botbeer.tool.sqlite_db as sqlite_db
import botbeer.tool.Config as Config
import os
...
class 功能包:
    ...
    async def 发送卡片(self, 文本):
        if not isinstance(文本, str):
            文本 = str(文本)
        await self.机器人.发送(文本, 类型="markdown")

    async def 发送HTML(self, 文本):
        if not isinstance(文本, str):
            文本 = str(文本)
        await self.机器人.发送(文本, 类型="html")

    async def 发送(self, 文本 = "", 图片 = ""):
        """图片支持(路径)(链接)(bytes图片)"""
        if not isinstance(文本, str):
            文本 = str(文本)
        """机器人发送消息"""
    ...
    def URL编码(self, 文本: str) -> str:
        """URL编码"""
        return urllib.parse.quote(文本)

    def URL解码(self, 文本: str) -> str:
        """URL解码"""
        return urllib.parse.unquote(文本)

    def HTML编码(self, 文本: str) -> str:
        """HTML编码"""
        return html.escape(文本)

    def HTML解码(self, 文本: str) -> str:
        """HTML解码"""
        return html.unescape(文本)

    def MD5(self, 文本: str) -> str:
        """MD5加密"""
        m = hashlib.md5()
    ...
    def Base64解码(self, 文本: str) -> str:
        """Base64解码"""
        return base64.b64decode(文本.encode("utf-8")).decode("utf-8")

    async def HTML画图(self, html_content: str) -> bytes:
        """HTML画图"""
        return await html_img.HTML画图(html_content)
